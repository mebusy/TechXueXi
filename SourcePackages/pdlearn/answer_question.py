...
def generate_tiku_data(
    quiz_type=None, tip=None, option=None, answer=None, question=None
):
    """
...
def find_available_quiz(quiz_type, driver_ans, uid):
    all_pages = driver_ans.driver.find_elements_by_css_selector(".ant-pagination-item")
    print(f"共找到 {len(all_pages)} 页")
    ...
    for p in range(
        0, pages, 1
    ):  # (从最后一页开始往前找做题)从前往后找题，专项答题等没有那么离谱
        time.sleep(0.5)
        ...
        for i in range(len(dati) - 1, -1, -1):  # 从最后一个遍历到第一个
            ...
            if "重新" in j.text or "满分" in j.text:
                ...
                return to_click
        page_next = driver_ans.driver.find_elements_by_css_selector(
            ".ant-pagination-next"
        )
        page_next[0].click()
...
def answer_question(
    ...
    print(
        "当前答题类型：" + quiz_type,
        "当前分数：" + str(scores[quiz_type]) + "，满分：" + str(score_all),
        "，当前cookies：" + str(cookies),
    )
    ...
    if scores[quiz_type] < score_all:  # 还没有满分，需要答题
        ...
        print("请保持窗口最大化\n" * 3)
        driver_ans.get_url("https://pc.xuexi.cn/notFound.html")
        ...
        if scores[quiz_type] < score_all:
            ...
            driver_ans.get_url("https://pc.xuexi.cn/points/exam-practice.html")
            refresh_my_page_count = 0
            # 页面title为积分则一直循环
            while not driver_ans.title_is("每日答题"):
                print("页面title不为积分，等待页面刷新提示")
                time.sleep(1)  # 等待页面刷新提示

            # while driver_ans.title_is("每日答题") and refresh_my_page_count < 20:
            #     print("页面title为积分，尝试刷新页面")
            #     time.sleep(1)  # 等待页面刷新提示
            ...
            #         print("button found, click it:", refresh_buttons[0].text)
                ...
                if to_click is not None:
                    ...
                    if driver_default is None:
                        try:
                            ...
                            print(
                                "driver_ans 在 answer_question 退出时出了一点小问题..."
                            )
                    else:
