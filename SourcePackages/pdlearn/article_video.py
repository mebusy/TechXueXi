...
def article(userId, cookies, article_pointer, scores):
    try:
        if (
            scores["article_num"] < const.article_num_all
            or scores["article_time"] < const.article_time_all
        ):
                        ...
                        for j in range(readarticle_time):
                            if random.random() > 0.5:
                                driver_article.go_js(
                                    "window.scrollTo(0, document.body.scrollHeight/120*{})".format(
                                        j
                                    )
                                )
                            print(
                                "\r文章数量学 xi 中，文章剩余{}篇,本篇剩余时间{}秒".format(
            ...
            while True:
                if scores["article_time"] < const.article_time_all and try_count < 10:
                    ...
                    remaining = (
                        (const.article_time_all - scores["article_time"]) * 1 * num_time
                    )
                    for i in range(remaining):
                        if random.random() > 0.5:
                            driver_article.go_js(
                                "window.scrollTo(0, document.body.scrollHeight/{}*{})".format(
                                    remaining, i
                                )
                            )
                        print(
                            "\r文章时长学 xi 中，文章总时长剩余{}秒".format(
                                remaining - i
                            ),
                            end="",
                        )
            ...
            if try_count < 10:
                ...
                print(
                    "文章学 xi 出现异常，请检查 user/article_video_index.json 文件记录"
                )
            driver_article.quit()
        else:
            print("文章之前学完了")
...
def video(userId, cookies, video_pointer, scores):
    try:
        if (
            ...
            or scores["video_time"] < const.video_time_all
        ):
                ...
                if scores["video_num"] < const.video_num_all and try_count < 10:
                    ...
                    for i in range(video_pointer, video_pointer + v_num):
                        ...
                        for j in range(watchvideo_time):
                            if random.random() > 0.5:
                                driver_video.go_js(
                                    "window.scrollTo(0, document.body.scrollHeight/180*{})".format(
                                        j
                                    )
                                )
                            print(
                                "\r视频数量学 xi 中，视频剩余{}个,本次剩余时间{}秒".format(
                                    video_pointer + v_num - i, watchvideo_time - j
                                ),
                                end="",
                            )
                            time.sleep(1)
            ...
            while True:
                if scores["video_time"] < const.video_time_all and try_count < 10:
                    ...
                    remaining = (
                        (const.video_time_all - scores["video_time"]) * 1 * num_time
                    )
                    for i in range(remaining):
                        if random.random() > 0.5:
                            driver_video.go_js(
            ...
            if try_count < 10:
                ...
                print(
                    "视频学 xi 出现异常，请检查 user/article_video_index.json 文件记录"
                )
            driver_video.quit()
        else:
            print("视频之前学完了")
